...
class CDMF(object):
    def __init__(self, num_all_users, num_all_movies, num_all_info, num_all_main_actors,
                 ...
                 num_main_actors, dim_hidden1, dim_hidden2, dim_lantent, filters_size_list,
                 num_filters, l2_reg_lambda_u, l2_reg_lambda_m, l2_reg_lambda_cnn,
                 l2_reg_lambda_info1, l2_reg_lambda_info2, l2_reg_lambda_actors1,
                 l2_reg_lambda_actors2):
        # embed params
        ...
        self.l2_reg_lambda_info1 = l2_reg_lambda_info1
        self.l2_reg_lambda_info2 = l2_reg_lambda_info2
        self.l2_reg_lambda_actors1 = l2_reg_lambda_actors1
        self.l2_reg_lambda_actors2 = l2_reg_lambda_actors2
    ...
    def construct_netword(self):
        ...
        movies_epsilon = self._embed_movies_epsilon(self.m_oids) # N*1*dim_lantent
        ...
        description = self._dense_cnn(droped_h)  # N*dim_hidden2
        ...
        info1 = self._dense_info1(embedded_info_flat)  # N*dim_hidden1
        info2 = self._dense_info2(info1)  # N*dim_hidden2
        ...
        actors1 = self._dense_actors1(embedded_actors_flat)  # N*dim_hidden1
        actors2 = self._dense_actors2(actors1)  # N*dim_hidden2

        # generate movie vecrors
        all_features = tf.concat(
            [description, info2, actors2], axis=1)  # N*(3*dim_hidden2)
        movies_mat = tf.expand_dims(self._dense_outputs(all_features), 1)  # N*1*dim_lantent
        ...
        with tf.variable_scope('loss', reuse=None):
            reg_loss = self.l2_reg_lambda_u * tf.nn.l2_loss(users_mat) + \
                self.l2_reg_lambda_m * tf.nn.l2_loss(movies_mat) + \
                self.l2_reg_lambda_cnn * self.l2_loss_cnn + \
                self.l2_reg_lambda_info1 * self.l2_loss_info1 + \
                self.l2_reg_lambda_info2 * self.l2_loss_info2 + \
                self.l2_reg_lambda_actors1 * self.l2_loss_actors1 + \
                self.l2_reg_lambda_actors2 * self.l2_loss_actors2
            self.loss_op = tf.add(
    ...
    def _dense_cnn(self, inputs, scope='dense_cnn', reuse=None):
        with tf.variable_scope(scope, reuse=reuse):
            W = tf.get_variable(name='W',
    ...
    def _dense_info1(self, inputs, scope='dense_info1', reuse=None):
        with tf.variable_scope(scope, reuse=reuse):
            W = tf.get_variable(name='W',
                                ...
                                initializer=tf.truncated_normal_initializer(mean=0.0, stddev=0.01))
            b = tf.get_variable(name='b',
                                shape=[self.dim_hidden1],
                                dtype=tf.float32,
                                initializer=tf.truncated_normal_initializer(mean=0.0, stddev=0.01))
            self.l2_loss_info1 = tf.nn.l2_loss(W)
            self.l2_loss_info1 += tf.nn.l2_loss(b)
            logits = tf.nn.xw_plus_b(inputs, W, b, name='logits')
            outputs = tf.nn.tanh(logits, name='outputs')
        return outputs
    ...
    def _dense_info2(self, inputs, scope='dense_info2', reuse=None):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            b = tf.get_variable(name='b',
                                ...
                                dtype=tf.float32,
                                initializer=tf.truncated_normal_initializer(mean=0.0, stddev=0.01))
            self.l2_loss_info2 = tf.nn.l2_loss(W)
            self.l2_loss_info2 += tf.nn.l2_loss(b)
            logits = tf.nn.xw_plus_b(inputs, W, b, name='logits')
            outputs = tf.nn.tanh(logits, name='outputs')
        return outputs
    ...
    def _dense_actors1(self, inputs, scope='dense_actors1', reuse=None):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            b = tf.get_variable(name='b',
                                ...
                                dtype=tf.float32,
                                initializer=tf.truncated_normal_initializer(mean=0.0, stddev=0.01))
            self.l2_loss_actors1 = tf.nn.l2_loss(W)
            self.l2_loss_actors1 += tf.nn.l2_loss(b)
            logits = tf.nn.xw_plus_b(inputs, W, b, name='logits')
            outputs = tf.nn.tanh(logits, name='outputs')
        return outputs
    ...
    def _dense_actors2(self, inputs, scope='dense_actors2', reuse=None):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            b = tf.get_variable(name='b',
                                ...
                                dtype=tf.float32,
                                initializer=tf.truncated_normal_initializer(mean=0.0, stddev=0.01))
            self.l2_loss_actors2 = tf.nn.l2_loss(W)
            self.l2_loss_actors2 += tf.nn.l2_loss(b)
            logits = tf.nn.xw_plus_b(inputs, W, b, name='logits')
            outputs = tf.nn.tanh(logits, name='outputs')
        return outputs

    def _dense_outputs(self, inputs, scope='dense_outputs', reuse=None):
        with tf.variable_scope(scope, reuse=reuse):
            W = tf.get_variable(name='W',
                                shape=[inputs.get_shape().as_list()[-1],
                                       self.dim_lantent],
            ...
            b = tf.get_variable(name='b',
                                ...
                                dtype=tf.float32,
                                initializer=tf.truncated_normal_initializer(mean=0.0, stddev=0.01))
            self.l2_loss_outputs = tf.nn.l2_loss(W)
            self.l2_loss_outputs += tf.nn.l2_loss(b)
            logits = tf.nn.xw_plus_b(inputs, W, b, name='logits')
            outputs = tf.nn.tanh(logits, name='outputs')
        return outputs
